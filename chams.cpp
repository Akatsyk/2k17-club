...
IMaterial* c_chams::get_material(int material) {
	...
	if (!init_materials) {
		...
		std::ofstream(sxor("csgo\\materials\\dsv_flat.vmt")) << R"("VertexLitGeneric"
		{
...
{
	bool preserve = ctx.m_settings.menu_chams_type == 3;

	IMaterial* mat = get_material(1);
	...
	debugdrawflat = csgo.m_material_system()->FindMaterial(("debug/debugdrawflat"), ("Model textures"));
...
int c_chams::work(void* context, void* state, ModelRenderInfo_t& info, matrix3x4_t* pCustomBoneToWorld)
{
	...
	Color hecolor = (ctx.m_settings.colors_chams_enemy_hidden);
	Color vecolor = (ctx.m_settings.colors_chams_enemy_viable);

	//Color htcolor = ctx.m_settings.colors_chams_teammate_hidden;
	//Color vtcolor = ctx.m_settings.colors_chams_teammate_viable;

	Color acolor = ctx.m_settings.colors_chams_local_desync;
	...
	if (classid == class_ids::CCSPlayer && strstr(info.pModel->szName, ("models/player")) != nullptr)
	{
		...
		if (entity != nullptr && entity->GetClientClass() != nullptr && !entity->m_bIsRagdoll() && !entity->IsDead() && entity->m_iHealth() > 0)
		{
			...
			else
			{
				...
				if (ctx.m_settings.chams_local_player && !fake_pos.IsZero() && ctx.m_local()->get_bone_pos(8).DistanceSquared(fake_pos) > 1/* && feature::anti_aim->enable_delay <= csgo.m_globals()->realtime && entity->m_bone_count() <= 128 && entity->m_bone_count() > 0*/)
				...
				//		c_chams::player_chams(original, d_settings, (entity->m_bIsScoped() && csgo.m_input()->is_in_tp()));
