	...
	if (best.player && ctx.m_local()) {
		...
		if (m_damage > 0) {
			...
			auto hc_result = !ctx.m_settings.aimbot_hitchance 
				|| ctx.m_settings.aimbot_hitchance && hit_chance(m_angle, m_aim, m_target, ctx.force_aimbot > 0 ? ctx.m_settings.aimbot_doubletap_hitchance_val : ctx.m_settings.aimbot_hitchance_val, best.hitbox, 1/*max(min(5, m_damage * 0.2f), 1)*/, m_best_hc);
