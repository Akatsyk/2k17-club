...
namespace Hooked
{
	...
	void __vectorcall CL_Move(float accumulated_extra_samples, bool bFinalTick)
	{
		...
		if (ctx.doubletap_now) {
			...
			int nCorrectedTick = TIME_TO_TICKS(0.03f) - (13 + csgo.m_client_state()->m_iChockedCommands) + csgo.m_globals()->tickcount + ctx.last_cmd_delta + 1;
