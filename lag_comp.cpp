...
void c_lagcomp::parse_player_data(C_Tickrecord* record, C_BasePlayer* m_player)
{
	...
	resolver_info->resolver_index = record->resolver_index;
	resolver_info->resolver_type = record->resolver_type;
...
void c_lagcomp::run_animation_data_resolver(C_Tickrecord* record, C_BasePlayer* m_player, C_Tickrecord* previous)
{
	...
	if (m_player->m_fFlags() & FL_ONGROUND && previous->entity_flags & FL_ONGROUND/* && !record->shot_this_tick && !previous->shot_this_tick*/)
	{
		...
		if (speed < 0.1f)
		{
			if (/*speed < 0.1f && */record->ground_accel_last_time != record->simulation_time && record->anim_layers[3].m_flWeight == 0.0f && record->anim_layers[3].m_flCycle == 0.0f && record->anim_layers[6].m_flWeight == 0.0f)
			{
		...
		else if ((record->anim_layers[12].m_flWeight < 0.01f || abs(previous->anim_layers[12].m_flWeight - record->anim_layers[12].m_flWeight) < 0.01f) && previous->anim_layers[6].m_nSequence == record->anim_layers[6].m_nSequence)
		{
			//2 = -1; 3 = 1; 1 = fake;
