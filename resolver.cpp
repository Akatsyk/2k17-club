...
void c_resolver::store_data(C_BasePlayer* m_player, C_Tickrecord* record, bool can_store)
{
	...
	/* update first desync matrix */
	{
		...
		memcpy(m_player->animation_layers_ptr(), r_log->resolver_layers_test[2], 0x38 * m_player->get_animation_layers_count());
		...
		if (!can_store || record->lag <= 1) 
			setup_side(r_log->simulated_animstates[1], record->leftmatrixes, r_log->resolver_layers_test[1], record->left_side, record->left_poses, 1, flags, poses, idx);
		else {
			if (delta < 0.0f) {
				setup_side(r_log->simulated_animstates[2], record->rightmatrixes, r_log->resolver_layers_test[2], record->right_side, record->right_poses, 1, flags, poses, idx);
			}
			else {
				setup_side(r_log->simulated_animstates[1], record->leftmatrixes, r_log->resolver_layers_test[1], record->left_side, record->left_poses, 1, flags, poses, idx);
			}
...

	{
		...
		memcpy(m_player->animation_layers_ptr(), r_log->resolver_layers_test[1], 0x38 * m_player->get_animation_layers_count());
		//csgo.m_globals()->framecount = TIME_TO_TICKS(record->simulation_time);
		//r_log->force_velocity = true;
		ctx.updating_resolver = true;
		m_player->update_clientside_animations();	/* update animations. */
		...
		if (!can_store || record->lag <= 1)
			setup_side(r_log->simulated_animstates[2], record->rightmatrixes, &r_log->resolver_layers_test[2][0], record->right_side, record->right_poses, 1, flags, poses, idx);
		else {
			if (delta < 0.0f) {
				setup_side(r_log->simulated_animstates[1], record->leftmatrixes, &r_log->resolver_layers_test[1][0], record->left_side, record->left_poses, 1, flags, poses, idx);
			}
			else {
	...
	record->accurate_anims = equal(r_log->resolver_layers[1][6], record->anim_layers[6]) && equal(r_log->resolver_layers[1][3], record->anim_layers[3]) && equal(r_log->resolver_layers[1][12], record->anim_layers[12])
		|| equal(r_log->resolver_layers[0][6], record->anim_layers[6]) && equal(r_log->resolver_layers[0][3], record->anim_layers[3]) && equal(r_log->resolver_layers[0][12], record->anim_layers[12])
		|| equal(r_log->resolver_layers[2][6], record->anim_layers[6]) && equal(r_log->resolver_layers[2][3], record->anim_layers[3]) && equal(r_log->resolver_layers[2][12], record->anim_layers[12]);
...
bool c_resolver::resolve_using_animations(C_BasePlayer* m_player, C_Tickrecord* record, int& result)
{
	...
	for (float i = 0.f; i <= 1.0f; i += 0.005f) {
		...
		float dist = m_player->GetSequenceMoveDist((*(CStudioHdr**)((uintptr_t)m_player + 0x294C)), anim_layer->m_nSequence);
...
bool c_resolver::select_next_side(C_BasePlayer* m_player, C_Tickrecord* record)
{
	...
	if ((next_resolving_type == 0 || r_log->next_resolving_index.empty()) && shots < 1)
	{
		...
		if (fabs(record->eye_angles.x) > 45.f)
		{
			...
			return next_resolving_type >= 1 || log->saved_info.fakeplayer || record->shot_this_tick || record->desync_delta < 40.f;
		}
	}
