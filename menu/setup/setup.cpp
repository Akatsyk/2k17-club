	...
	const auto main_window = new window(Vector2D(50, 50), Vector2D(630, 500)); {
		...
		const auto antiaim = new tab("anti-aim"); {
			const auto antiaim_section_1 = new section(); {
				...
				antiaim_section_1->add_slider("side limit", 0, 60, &ctx.m_settings.anti_aim_fake_limit, L"°");
			}
		...
		const auto players = new tab("players"); {
			...
			{
				players_section_1->add_checkbox("enabled", &ctx.m_settings.visuals_enabled);
...
//					ImGui::ColorPickerBox("#nigton", ctx.m_settings.colors_skeletons_enemy, nullptr, nullptr, nullptr);
				...
				players_section_1->add_checkbox("dormant enemies", &ctx.m_settings.esp_dormant);

				players_section_1->add_checkbox("name", &ctx.m_settings.esp_name);
				//players_section_1->add_colorpicker("name color", ctx.m_settings.colors_esp_name, false);
				players_section_1->add_checkbox("health", &ctx.m_settings.esp_health);

				players_section_1->add_checkbox("weapon", &ctx.m_settings.esp_weapon);
			...
			const auto players_section_2 = new section(); {
				...
				//players_section_2->add_checkbox("enable desync", &ctx.m_settings.chams_backtrack, &ctx.m_settings.changing_chams[3]);
				players_section_2->add_checkbox("enable chams", &ctx.m_settings.chams_enemy, &ctx.m_settings.changing_chams[0]);

				players_section_2->add_checkbox("enemy", &ctx.m_settings.chams_walls, &ctx.m_settings.changing_chams[0]);
				players_section_2->add_colorpicker("visible color", &ctx.m_settings.colors_chams_enemy_viable, &ctx.m_settings.changing_chams[0]);
				players_section_2->add_colorpicker("invisible color", &ctx.m_settings.colors_chams_enemy_hidden, &ctx.m_settings.changing_chams[0]);
				...
				players_section_2->add_checkbox("enable fake", &ctx.m_settings.chams_local_player, &ctx.m_settings.changing_chams[3]);
		...
		const auto visuals = new tab("visuals"); {
			...
			const auto visuals_section_2 = new section(); {
				visuals_section_2->add_checkbox("autowall crosshair", &ctx.m_settings.visuals_autowall_crosshair);
				visuals_section_2->add_checkbox("remove visual recoil", &ctx.m_settings.visuals_no_recoil);
				visuals_section_2->add_checkbox("remove visual smoke", &ctx.m_settings.visuals_no_smoke);
				visuals_section_2->add_checkbox("remove visual flash", &ctx.m_settings.visuals_no_flash);
		...
		const auto movement = new tab("movement"); {
			const auto movement_section_1 = new section(); {
				...
				movement_section_1->add_checkbox("quick stop", &ctx.m_settings.aimbot_autostop);
				movement_section_1->add_checkbox("force accuracy", &ctx.m_settings.autostop_force_accuracy);
				movement_section_1->add_checkbox("between shots", &ctx.m_settings.autostop_only_when_shooting);
				movement_section_1->add_combobox("autostop method", &ctx.m_settings.autostop_type, { "slow motion", "full stop" });
				...
				movement_section_1->add_combobox("leg movement options", &ctx.m_settings.anti_aim_leg_movement, { "default", "always slide", "never slide", "breaker" });
			}
